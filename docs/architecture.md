...
- **美股代理漲跌 = 超出「官方淨值那場收盤」的增量**,不是超出「最近一次收盤」——estimateNav 在台股盤中結出,只看得到 `closingNavDate`(ET T−1)那場。美股一收盤到台股次日開盤前(平日台北 04:00–09:00、整個週末),最近收盤就比官方淨值新一場,需改錨回去,否則整場漲跌會被吃掉。
...
3. 基準收盤要對齊官方淨值**結算時所用**的那場美股 regular 收盤:對得太舊會重複計算,對得太新會整場漏算(見 [ADR 0002](adr/0002-重估模型.md))。
